...
func solve(m [][]int) int {
	...
	for pq.Len() > 0 {
		...
		if point.x == len(m)-1 && point.y == len(m[0])-1 && point.straightLine >= 4 {
			if point.pathValue < minValue {
				// if minValue != 10000000000000000 {
				// 	fmt.Println(point.pathValue)
				// 	panic("second min")
				// }
		...
		if point.dir == 'L' {
			...
			if point.straightLine+1 >= 4 && point.straightLine+1 <= 10 {
		...
		for _, delta := range deltas {
			...
			if !isWithinBounds(m, p) || p.straightLine > 9 {
				continue
			}
